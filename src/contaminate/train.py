...
if not pt.started():
    pt.init()
from rankers import Dot, RankerTrainer, RankerArguments, DotDataCollator, seed_everything, CatDataCollator, Cat
...
def train(model_name_or_path : str, 
    ...
    collate_fn = collate_fn_constructor(AutoTokenizer.from_pretrained(model_name_or_path))
    ...
    sched = get_linear_schedule_with_warmup(opt, int(args.warmup_ratio * max_steps), max_steps) if args.warmup_ratio > 0 else None
    ...
    if resume_from_checkpoint:
        ...
        checkpoints = [int(x.split('-')[1]) for x in checkpoints if x.startswith('checkpoint')]
